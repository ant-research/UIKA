...
class PixelLoss(nn.Module):
    ...
    # @torch.compile
    # def forward(self, x, y, mask, conf_sigma=None, only_sym_conf=False):
    #     """
    ...
    #     mask = rearrange(mask, "n m c h w -> (n m) c h w")

    #     pixel_loss = self.loss_fn(x, y)
    #     masked_pixel_loss = pixel_loss * mask

    #     image_loss = masked_pixel_loss.mean(dim=[1, 2, 3])
