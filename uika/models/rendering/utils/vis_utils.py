...
def render_mesh(mesh, face, cam_param, bkg, blend_ratio=1.0, return_bg_mask=False, R=None, T=None, return_fragments=False):
    ...
    # ------------------Temp Modify -------------------
    # mesh_rgb = images[0,:,:,:3].cpu().numpy()  # (H, W, 3)
    # save_dir = 'debug_vis/flame_mesh_vis'
    # os.makedirs(save_dir, exist_ok=True)
    # img_num = len(os.listdir(save_dir))
    # from PIL import Image
    # # save rgba image
    # Image.fromarray((mesh_rgb * 255).astype(np.uint8)).save(os.path.join(save_dir, f'{img_num:05d}_rgba.png'))
    # ------------------Temp Modify -------------------
